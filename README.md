...
# This document under development
...
6. A directory is created with the name "Demo_Website". Navigate to that directory. Inside that directory initially we will have a python file named "manage.py" and a directory with the same name.
    ...
    6-b. The directory's primary use is to connect the other 
	applications that you are going to create for the web 
	application. We can develop applications (example a 
	blog, a chatroom) and connect all of them to the main 
	web application by modifying the files and contents 
	here. This is like the main directory that points to 
	all different web applications inside a website. It 
	consists of 5 files:
...
8. A new directory is created again with the name "home". It contains one directory and 6 files. These are for configuring the application "home".
	...
	models.py ==> Every application is based on the 
		      models in the Django. It is like creating 		      and mapping to a database table.
...
19(Optional). Instead of using a template to add the data into the database we can use the django's interactive shell. It is easy and fast. To open the shell run command 
...
24. Do not mind the name in the webpage which is plural (Products) but you have registed singular (Product). There is an in-built function that does this. We can override it if we want later on.
...
34. The css or javascript files and some headings (like title) or navigation bar are common for most of the html pages in an application. So rather than copy pasting the -- importing -- lines for every html file we can write those lines in a separate html file and import that one file in every html file in that application with one or few lines of code.
...
43. Now modifying the view to get the data by clicking the register button. It looks like this:
    ...
    *Same usernames must not be given. Passwords can be duplicated. To check your passwords if you are stuck:
...
49. Now adding the login functionality.	Add the url pattern in urls file.
